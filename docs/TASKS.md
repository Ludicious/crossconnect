...
| Bulk/multi-select delete across inventory (racks, systems) and connection grid rows | Feature | P1-high | — | Reported 2026-09-03, confirmed by real use: single-item delete + recycle bin works cleanly (verified after the systems recycle-bin fix), but cleaning up a large bad import (dozens of racks/systems) one-at-a-time with a confirm prompt each time is impractical. Originally logged as grid-only ("Batch delete of connection grid rows") — broadened after hands-on testing to cover Inventory list views too (rack list, system list). Needs multi-select checkboxes + bulk delete action, consistent with the existing per-row delete pattern. Raised priority given it's the direct blocker on cleaning up current test data. |
...
_Verified against `git log` and `debug_sim` (148/148 passing as of last update, 2026-09-03)._
