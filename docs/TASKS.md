...
| Build sequencing decision: scope (a), (b), or (c) | Investigation / Product decision | P1-high | — | Panel presented three scope levels 2026-09-03. **(a) Minimum viable:** `deleted_at` + fixed `delete_rack()` + fixed purge cascade + WO-cancel semantics — unblocks the immediate pilot breakage but explicitly does *not* make occupancy trustworthy (no uniqueness constraint). Panel cautioned against describing (a) as "fixing occupancy" — it only fixes deletion. **(b) Proposal as written:** (a) + derived green/yellow/blue occupancy + sparse red-status table + uniqueness constraint. **(c)** = (b) + trunk modeling (empty table, lazy discovery, cross-rack delete guard). Panel's read: (b) and (c) together are reasonably scoped and **additive, not a remodel**, given lazy discovery avoids backfill. Decision needed: ship (a) immediately to unblock cleanup and follow with (b)+(c), or go straight to (b) as the v1 given the uniqueness constraint is a must-ship. |
